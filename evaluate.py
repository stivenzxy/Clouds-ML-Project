"""
Punto C — Evaluación del modelo final en el conjunto de validación.

Carga el mejor checkpoint guardado por `train_final.py` y evalúa sobre
`val_split.csv` (intocado hasta ahora).

Reporta, por cada clase de nube (Fish, Flower, Gravel, Sugar):
...
Salidas en `results/`:
- `metrics_per_class.csv`: tabla con todas las métricas por clase.
- `confusion_matrices.png`: figura con las 4 matrices.
- `metrics_summary.md`: tabla en markdown lista para el paper.
- `predictions_val.csv`: predicciones (probabilidades y binarias) para inspección.
"""
...
def predict(model, loader, device):
    """Devuelve probabilidades, predicciones binarias y etiquetas verdaderas."""
    model.eval()
    all_probs = []
    all_labels = []
...
def compute_metrics_per_class(y_true, y_pred):
    """Calcula accuracy, precision, recall, F1 y matriz de confusión por clase."""
    rows = []
    cms = {}
    ...
    # Promedios globales (por filas, no ponderado por soporte).
    macro_row = {
        'Clase': 'MACRO',
...
def plot_confusion_matrices(cms, save_path):
    """Genera una figura con las 4 matrices de confusión."""
...
def export_markdown_table(df, save_path):
    """Exporta la tabla de métricas en formato Markdown para pegar en el paper."""
    rows = []
...
def main():
    ...
    # Guardar predicciones (para el punto D y trazabilidad).
